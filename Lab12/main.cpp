...
// Multi-Viewport
double current_width = 500.0f;
double current_height = 500.0f;
double spin = 0;
void draw_can();
void draw_disk();
void draw_teapot();
...
int main(int argc, char** argv) {
    ...
    glutCreateWindow("12201933 이승은 Lab12");
    init(); // -> 사용자 초기화 함수
...
void draw_disk(void) {
    ...
    glPushMatrix();
    gluPartialDisk(qobj, 2, 5, 30, 3, 90, 270);
    glPopMatrix();
...
void draw_text() {
    ...
    draw_string(GLUT_BITMAP_TIMES_ROMAN_24, "Graphics are cool!!", -8, 2, 1, 1, 0);
    glColor3f(0.5f, 1.0f, 0);
    draw_string(GLUT_BITMAP_HELVETICA_18, "Multi-Viewport Example!!", -8, 0, 0, 1, 1);
    glFlush();
